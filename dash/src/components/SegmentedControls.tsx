...
export default function SegmentedControls<T>({ id, className, options, selected, onSelect }: Props<T>) {
	return (
		<LayoutGroup>
			<div id={id} className={clsx("m-0 inline-flex h-fit justify-between rounded-lg bg-transparent p-0.5", className)}>
						...
						>
							...
							>
								{isActive && (
									<motion.div
										layoutId={`segment-${id}`}
										className="absolute bottom-0 left-0 right-0 top-0 z-[1] rounded-md bg-zinc-200"
									/>
								)}
